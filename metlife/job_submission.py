...
def submitJob(model_name):
	...
	print (str(datetime.datetime.now())+ " " +str(build_api_response.json()))
...
def getJobs():
	current_status = 'done'
	JOB_API_URL = host + ":" + port + "/" + job_api + "&project=" + project
...
for myModel in myModels:
	while True:
		...
		ret = getJobs()
		if ret == False:
			print (str(datetime.datetime.now())+ " Jobs running, waiting for a minute before checking again..")
			time.sleep(60)
			continue
		elif ret == True:
			print (str(datetime.datetime.now())+ " No job running now")
			break
