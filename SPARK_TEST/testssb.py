...
print ("SELECT D_YEAR, C_MKTSEGMENT, C_REGION, sum(LO_QUANTITY), sum(LO_REVENUE) from JOINED DATA_FRAME GROUP BY D_YEAR, C_MKTSEGMENT, C_REGION ORDER BY LO_REVENUE\n")

st = time.time()
df.groupBy('D_YEAR','C_MKTSEGMENT','C_REGION','C_NATION','C_CITY').sum('LO_QUANTITY','LO_REVENUE').orderBy(sum('LO_REVENUE')).show()
et = time.time()
exectime = et - st
print ("####### QRY EXEC TIME: " + str(exectime) + " seconds ###########")
time.sleep(180)
